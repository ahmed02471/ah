/*!
 * jsQR.js — نسخة محلية (مُجمَّعة يدوياً من مصدر cozmo/jsQR، رخصة Apache-2.0)
 * تم تجميعها محلياً لأن سكربت CDN لا يعمل على شبكة هذا الجهاز (لا يصل للإنترنت).
 * ملاحظة: دعم نمط Kanji (شيفت-JIS) مُبسّط/فارغ لأن رموز QR في هذا النظام
 * أبجدية-رقمية (Alphanumeric) أو Byte فقط — لا تُستخدم بيانات Kanji هنا.
 */
(function (global) {
  "use strict";
  console.log('[jsQR] تم تحميل الملف المحلي — نسخة التصحيح rev4');
  ...
  // ── GenericGF / GenericGFPoly (Reed-Solomon math) ───────────────
  ...
  class GenericGFPoly {
    constructor(field, coefficients) {
      ...
      if (coefficientsLength > 1 && coefficients[0] === 0) {
        ...
        if (firstNonZero === coefficientsLength) {
          ...
          for (let i = 0; i < this.coefficients.length; i++) this.coefficients[i] = coefficients[firstNonZero + i];
        }
      } else {
        this.coefficients = coefficients;
      }
    }
    degree() { return this.coefficients.length - 1; }
    ...
    evaluateAt(a) {
      ...
      const size = this.coefficients.length;
      if (a === 1) {
        this.coefficients.forEach((c) => { result = addOrSubtractGF(result, c); });
        return result;
      }
      result = this.coefficients[0];
      for (let i = 1; i < size; i++) result = addOrSubtractGF(this.field.multiply(a, result), this.coefficients[i]);
      return result;
    }
  }
  ...
  function runEuclideanAlgorithm(field, a, b, R) {
    ...
    let rLast = a, r = b, tLast = field.zero, t = field.one;
    while (r.degree() >= R / 2) {
      const rLastLast = rLast, tLastLast = tLast;
      rLast = r; tLast = t;
  ...
  function findErrorLocations(field, errorLocator) {
    ...
    for (let i = 1; i < field.size && errorCount < numErrors; i++) {
      if (errorLocator.evaluateAt(i) === 0) { result[errorCount] = field.inverse(i); errorCount++; }
    }
  ...
  function findErrorMagnitudes(field, errorEvaluator, errorLocations) {
    ...
    for (let i = 0; i < s; i++) {
      ...
      let denominator = 1;
      for (let j = 0; j < s; j++) {
        if (i !== j) denominator = field.multiply(denominator, addOrSubtractGF(1, field.multiply(errorLocations[j], xiInverse)));
      }
  ...
  class Matrix {
    ...
    get(x, y) { return this.data[y * this.width + x]; }
    set(x, y, value) { this.data[y * this.width + x] = value; }
  }
  ...
  function binarize(data, width, height, returnInverted) {
    ...
    for (let verticalRegion = 0; verticalRegion < verticalRegionCount; verticalRegion++) {
      for (let hortizontalRegion = 0; hortizontalRegion < horizontalRegionCount; hortizontalRegion++) {
        ...
        for (let xRegion = -2; xRegion <= 2; xRegion++) {
          for (let yRegion = -2; yRegion <= 2; yRegion++) sum += blackPoints.get(left + xRegion, top + yRegion);
        }
    ...
    if (returnInverted) return { binarized, inverted };
    return { binarized };
  }
  ...
  // ── decodeData (تفسير البيانات بعد تصحيح الأخطاء) ────────────────
  // ملاحظة: جدول Shift-JIS فارغ عمداً — نمط Kanji غير مستخدم في رموز QR لهذا النظام.
  const shiftJISTable = {};
  ...
  function decodeKanji(stream, size) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (c < 0x1F00) c += 0x8140; else c += 0xC140;
      bytes.push(c >> 8, c & 0xFF);
      text += String.fromCharCode(shiftJISTable[c] || 0);
    }
    return { bytes, text };
  }

  function decodeQRData(data, version) {
    const stream = new BitStream(data);
    const size = version <= 9 ? 0 : version <= 26 ? 1 : 2;
    const result = { text: "", bytes: [], chunks: [], version };
    while (stream.available() >= 4) {
      ...
      if (mode === ModeByte.Terminator) {
        ...
        if (stream.readBits(1) === 0) {
          result.chunks.push({ type: Mode.ECI, assignmentNumber: stream.readBits(7) });
        } else if (stream.readBits(1) === 0) {
          result.chunks.push({ type: Mode.ECI, assignmentNumber: stream.readBits(14) });
        } else if (stream.readBits(1) === 0) {
          result.chunks.push({ type: Mode.ECI, assignmentNumber: stream.readBits(21) });
        } else {
          result.chunks.push({ type: Mode.ECI, assignmentNumber: -1 });
        }
      } else if (mode === ModeByte.Numeric) {
        const r = decodeNumeric(stream, size);
        result.text += r.text; result.bytes.push(...r.bytes);
        result.chunks.push({ type: Mode.Numeric, text: r.text });
      } else if (mode === ModeByte.Alphanumeric) {
        const r = decodeAlphanumeric(stream, size);
        result.text += r.text; result.bytes.push(...r.bytes);
        result.chunks.push({ type: Mode.Alphanumeric, text: r.text });
      } else if (mode === ModeByte.Byte) {
        const r = decodeByte(stream, size);
        result.text += r.text; result.bytes.push(...r.bytes);
        result.chunks.push({ type: Mode.Byte, bytes: r.bytes, text: r.text });
      } else if (mode === ModeByte.Kanji) {
        const r = decodeKanji(stream, size);
        result.text += r.text; result.bytes.push(...r.bytes);
        result.chunks.push({ type: Mode.Kanji, bytes: r.bytes, text: r.text });
      }
    }
    if (stream.available() === 0 || stream.readBits(stream.available()) === 0) return result;
  }

  // ── VERSIONS (جدول إصدارات QR 1-40) ──────────────────────────────
  const VERSIONS = [
    { infoBits: null, versionNumber: 1, alignmentPatternCenters: [], errorCorrectionLevels: [
    ...
    { infoBits: null, versionNumber: 3, alignmentPatternCenters: [6, 22], errorCorrectionLevels: [
    ...
    { infoBits: null, versionNumber: 6, alignmentPatternCenters: [6, 34], errorCorrectionLevels: [
    ...
    { infoBits: 0x07C94, versionNumber: 7, alignmentPatternCenters: [6, 22, 38], errorCorrectionLevels: [
    ...
    { infoBits: 0x085BC, versionNumber: 8, alignmentPatternCenters: [6, 24, 42], errorCorrectionLevels: [
    ...
    { infoBits: 0x09A99, versionNumber: 9, alignmentPatternCenters: [6, 26, 46], errorCorrectionLevels: [
    ...
    { infoBits: 0x0A4D3, versionNumber: 10, alignmentPatternCenters: [6, 28, 50], errorCorrectionLevels: [
    ...
    { infoBits: 0x0BBF6, versionNumber: 11, alignmentPatternCenters: [6, 30, 54], errorCorrectionLevels: [
    ...
    { infoBits: 0x0C762, versionNumber: 12, alignmentPatternCenters: [6, 32, 58], errorCorrectionLevels: [
    ...
    { infoBits: 0x0D847, versionNumber: 13, alignmentPatternCenters: [6, 34, 62], errorCorrectionLevels: [
    ...
    { infoBits: 0x0E60D, versionNumber: 14, alignmentPatternCenters: [6, 26, 46, 66], errorCorrectionLevels: [
    ...
    { infoBits: 0x0F928, versionNumber: 15, alignmentPatternCenters: [6, 26, 48, 70], errorCorrectionLevels: [
    ...
    { infoBits: 0x10B78, versionNumber: 16, alignmentPatternCenters: [6, 26, 50, 74], errorCorrectionLevels: [
    ...
    { infoBits: 0x1145D, versionNumber: 17, alignmentPatternCenters: [6, 30, 54, 78], errorCorrectionLevels: [
    ...
    { infoBits: 0x12A17, versionNumber: 18, alignmentPatternCenters: [6, 30, 56, 82], errorCorrectionLevels: [
    ...
    { infoBits: 0x13532, versionNumber: 19, alignmentPatternCenters: [6, 30, 58, 86], errorCorrectionLevels: [
    ...
    { infoBits: 0x149A6, versionNumber: 20, alignmentPatternCenters: [6, 34, 62, 90], errorCorrectionLevels: [
    ...
    { infoBits: 0x15683, versionNumber: 21, alignmentPatternCenters: [6, 28, 50, 72, 94], errorCorrectionLevels: [
    ...
    { infoBits: 0x168C9, versionNumber: 22, alignmentPatternCenters: [6, 26, 50, 74, 98], errorCorrectionLevels: [
    ...
    { infoBits: 0x177EC, versionNumber: 23, alignmentPatternCenters: [6, 30, 54, 74, 102], errorCorrectionLevels: [
    ...
    { infoBits: 0x18EC4, versionNumber: 24, alignmentPatternCenters: [6, 28, 54, 80, 106], errorCorrectionLevels: [
    ...
    { infoBits: 0x191E1, versionNumber: 25, alignmentPatternCenters: [6, 32, 58, 84, 110], errorCorrectionLevels: [
    ...
    { infoBits: 0x1AFAB, versionNumber: 26, alignmentPatternCenters: [6, 30, 58, 86, 114], errorCorrectionLevels: [
    ...
    { infoBits: 0x1B08E, versionNumber: 27, alignmentPatternCenters: [6, 34, 62, 90, 118], errorCorrectionLevels: [
    ...
    { infoBits: 0x1CC1A, versionNumber: 28, alignmentPatternCenters: [6, 26, 50, 74, 98, 122], errorCorrectionLevels: [
    ...
    { infoBits: 0x1D33F, versionNumber: 29, alignmentPatternCenters: [6, 30, 54, 78, 102, 126], errorCorrectionLevels: [
    ...
    { infoBits: 0x1ED75, versionNumber: 30, alignmentPatternCenters: [6, 26, 52, 78, 104, 130], errorCorrectionLevels: [
    ...
    { infoBits: 0x1F250, versionNumber: 31, alignmentPatternCenters: [6, 30, 56, 82, 108, 134], errorCorrectionLevels: [
    ...
    { infoBits: 0x209D5, versionNumber: 32, alignmentPatternCenters: [6, 34, 60, 86, 112, 138], errorCorrectionLevels: [
    ...
    { infoBits: 0x216F0, versionNumber: 33, alignmentPatternCenters: [6, 30, 58, 86, 114, 142], errorCorrectionLevels: [
    ...
    { infoBits: 0x228BA, versionNumber: 34, alignmentPatternCenters: [6, 34, 62, 90, 118, 146], errorCorrectionLevels: [
    ...
    { infoBits: 0x2379F, versionNumber: 35, alignmentPatternCenters: [6, 30, 54, 78, 102, 126, 150], errorCorrectionLevels: [
    ...
    { infoBits: 0x24B0B, versionNumber: 36, alignmentPatternCenters: [6, 24, 50, 76, 102, 128, 154], errorCorrectionLevels: [
    ...
    { infoBits: 0x2542E, versionNumber: 37, alignmentPatternCenters: [6, 28, 54, 80, 106, 132, 158], errorCorrectionLevels: [
    ...
    { infoBits: 0x26A64, versionNumber: 38, alignmentPatternCenters: [6, 32, 58, 84, 110, 136, 162], errorCorrectionLevels: [
    ...
    { infoBits: 0x27541, versionNumber: 39, alignmentPatternCenters: [6, 26, 54, 82, 110, 138, 166], errorCorrectionLevels: [
    ...
    { infoBits: 0x28C69, versionNumber: 40, alignmentPatternCenters: [6, 30, 58, 86, 114, 142, 170], errorCorrectionLevels: [
  ...
  // ── decoder (قراءة المصفوفة، التحقق من التنسيق، فك التشفير) ──────
  function numBitsDiffering(x, y) {
    let z = x ^ y;
    let bitCount = 0;
    while (z) { bitCount++; z &= z - 1; }
    return bitCount;
  }
      ...
      for (let i = 0; i < dimension; i++) {
        ...
        for (let columnOffset = 0; columnOffset < 2; columnOffset++) {
          ...
          if (!functionPatternMask.get(x, y)) {
            ...
            if (dataMask({ y, x })) bit = !bit;
            currentByte = pushBit(bit, currentByte);
            if (bitsRead === 8) { codewords.push(currentByte); bitsRead = 0; currentByte = 0; }
          }
        }
      }
      readingUp = !readingUp;
  ...
  function readVersion(matrix) {
    ...
    if (provisionalVersion <= 6) return VERSIONS[provisionalVersion - 1];
    let topRightVersionBits = 0;
    for (let y = 5; y >= 0; y--) {
      for (let x = dimension - 9; x >= dimension - 11; x--) topRightVersionBits = pushBit(matrix.get(x, y), topRightVersionBits);
    }
    let bottomLeftVersionBits = 0;
    for (let x = 5; x >= 0; x--) {
      for (let y = dimension - 9; y >= dimension - 11; y--) bottomLeftVersionBits = pushBit(matrix.get(x, y), bottomLeftVersionBits);
    }
    ...
    for (const version of VERSIONS) {
      ...
      if (difference < bestDifference) { bestVersion = version; bestDifference = difference; }
  ...
  function readFormatInformation(matrix) {
    ...
    for (let x = 0; x <= 8; x++) {
      if (x !== 6) topLeftFormatInfoBits = pushBit(matrix.get(x, 8), topLeftFormatInfoBits);
    }
    for (let y = 7; y >= 0; y--) {
      if (y !== 6) topLeftFormatInfoBits = pushBit(matrix.get(8, y), topLeftFormatInfoBits);
    }
    const dimension = matrix.height;
    let topRightBottomRightFormatInfoBits = 0;
    for (let y = dimension - 1; y >= dimension - 7; y--) topRightBottomRightFormatInfoBits = pushBit(matrix.get(8, y), topRightBottomRightFormatInfoBits);
    for (let x = dimension - 8; x < dimension; x++) topRightBottomRightFormatInfoBits = pushBit(matrix.get(x, 8), topRightBottomRightFormatInfoBits);
    let bestDifference = Infinity;
    let bestFormatInfo = null;
    for (const { bits, formatInfo } of FORMAT_INFO_TABLE) {
      if (bits === topLeftFormatInfoBits || bits === topRightBottomRightFormatInfoBits) return formatInfo;
      let difference = numBitsDiffering(topLeftFormatInfoBits, bits);
      if (difference < bestDifference) { bestFormatInfo = formatInfo; bestDifference = difference; }
  ...
  function getDataBlocks(codewords, version, ecLevel) {
    ...
    for (let i = 0; i < shortBlockSize; i++) {
      for (const dataBlock of dataBlocks) dataBlock.codewords.push(codewords.shift());
    }
  ...
  function decodeMatrix(matrix) {
    ...
    try {
      return decodeQRData(resultBytes, version.versionNumber);
    } catch (e) { return null; }
  }
  ...
  // ── locator (تحديد مواقع أنماط الزوايا الثلاثة + المحاذاة) ────────
  ...
  function countBlackWhiteRunTowardsPoint(origin, end, matrix, length) {
    ...
    if (steep) {
      fromX = Math.floor(origin.y); fromY = Math.floor(origin.x);
      toX = Math.floor(end.y); toY = Math.floor(end.x);
    } else {
      fromX = Math.floor(origin.x); fromY = Math.floor(origin.y);
      toX = Math.floor(end.x); toY = Math.floor(end.y);
    }
  ...
  function scoreBlackWhiteRun(sequence, ratios) {
    ...
    let error = 0;
    ratios.forEach((ratio, i) => { error += (sequence[i] - ratio * averageSize) ** 2; });
  ...
  function scorePattern(point, ratios, matrix) {
    try {
      ...
      const topLeftPoint = { x: Math.max(0, point.x - point.y) - 1, y: Math.max(0, point.y - point.x) - 1 };
      const topLeftBottomRightRun = countBlackWhiteRun(point, topLeftPoint, matrix, ratios.length);
      const bottomLeftPoint = { x: Math.min(matrix.width, point.x + point.y) + 1, y: Math.min(matrix.height, point.y + point.x) + 1 };
      const bottomLeftTopRightRun = countBlackWhiteRun(point, bottomLeftPoint, matrix, ratios.length);
      const horzError = scoreBlackWhiteRun(horizontalRun, ratios);
      const vertError = scoreBlackWhiteRun(verticalRun, ratios);
      const diagDownError = scoreBlackWhiteRun(topLeftBottomRightRun, ratios);
      const diagUpError = scoreBlackWhiteRun(bottomLeftTopRightRun, ratios);
      const ratioError = Math.sqrt(horzError.error * horzError.error + vertError.error * vertError.error +
        diagDownError.error * diagDownError.error + diagUpError.error * diagUpError.error);
      const avgSize = (horzError.averageSize + vertError.averageSize + diagDownError.averageSize + diagUpError.averageSize) / 4;
      const sizeError = ((horzError.averageSize - avgSize) ** 2 + (vertError.averageSize - avgSize) ** 2 +
        (diagDownError.averageSize - avgSize) ** 2 + (diagUpError.averageSize - avgSize) ** 2) / avgSize;
  ...
  function findAlignmentPattern(matrix, alignmentPatternQuads, topRight, topLeft, bottomLeft) {
    ...
    const bottomRightFinderPattern = { x: topRight.x - topLeft.x + bottomLeft.x, y: topRight.y - topLeft.y + bottomLeft.y };
  ...
  function locate(matrix) {
      ...
      for (let x = -1; x <= matrix.width; x++) {
        ...
        if (v === lastBit) {
          ...
          if (validAlignmentPattern) {
            ...
            if (matchingQuads.length > 0) matchingQuads[0].bottom = line;
            else activeAlignmentPatternQuads.push({ top: line, bottom: line });
          }
        }
      }
    ...
    console.log('[jsQR] locate: عدد المرشحين الخام (quads):', finderPatternQuads.length,
      '— عدد المجموعات المرشحة:', finderPatternGroups.length,
      finderPatternGroups[0] ? ('— أفضل مجموعة score=' + finderPatternGroups[0].score.toFixed(2) +
        ' sizes=[' + finderPatternGroups[0].points.map(p => p.size.toFixed(1)).join(',') + ']' +
        ' xy=[' + finderPatternGroups[0].points.map(p => '(' + p.x.toFixed(0) + ',' + p.y.toFixed(0) + ')').join(',') + ']') : '');

    if (finderPatternGroups.length === 0) {
      console.log('[jsQR] locate: لا توجد مجموعات أنماط زوايا (finder patterns) — لم يتم العثور على أي نمط زاوية صالح');
      return null;
    }
    ...
    if (result.length === 0) {
      console.log('[jsQR] locate: وُجدت أنماط زوايا لكن فشل تحديد نمط المحاذاة/الأبعاد (findAlignmentPattern)');
      return null;
    }
    return result;
  }

  // ── extractor (تحويل منظوري لاستخراج مصفوفة QR نظيفة) ─────────────
  function squareToQuadrilateral(p1, p2, p3, p4) {
    const dx3 = p1.x - p2.x + p3.x - p4.x;
    const dy3 = p1.y - p2.y + p3.y - p4.y;
    if (dx3 === 0 && dy3 === 0) {
      return { a11: p2.x - p1.x, a12: p2.y - p1.y, a13: 0, a21: p3.x - p2.x, a22: p3.y - p2.y, a23: 0, a31: p1.x, a32: p1.y, a33: 1 };
    }
    const dx1 = p2.x - p3.x;
    const dx2 = p4.x - p3.x;
    const dy1 = p2.y - p3.y;
    ...
    return {
      a11: p2.x - p1.x + a13 * p2.x, a12: p2.y - p1.y + a13 * p2.y, a13,
      a21: p4.x - p1.x + a23 * p4.x, a22: p4.y - p1.y + a23 * p4.y, a23,
      a31: p1.x, a32: p1.y, a33: 1,
    };
  }

  function quadrilateralToSquare(p1, p2, p3, p4) {
    // المصفوفة المساعدة (adjugate) تُستخدم كمعكوس هنا
  ...
  function extract(image, location) {
    const qToS = quadrilateralToSquare(
      { x: 3.5, y: 3.5 }, { x: location.dimension - 3.5, y: 3.5 },
      { x: location.dimension - 6.5, y: location.dimension - 6.5 }, { x: 3.5, y: location.dimension - 3.5 },
    );
  ...
  // ── index (نقطة الدخول الرئيسية jsQR) ──────────────────────────────
  function scan(matrix) {
    const locations = locate(matrix);
    if (!locations) return null;
    console.log('[jsQR] scan: locate() وجد', locations.length, 'موقع/مواقع محتملة — جارٍ المحاولة استخراج وفك التشفير');
    for (const location of locations) {
      const extracted = extract(matrix, location);
      const decoded = decodeQR(extracted.matrix);
      if (!decoded) console.log('[jsQR] scan: فشل decodeQR لهذا الموقع (بُعد:', location.dimension, ')');
      if (decoded) {
        return {
          ...
          location: {
            ...
            bottomRightAlignmentPattern: location.alignmentPattern || undefined,
          },
        };
      }
    }
  ...
  const defaultOptions = { inversionAttempts: "attemptBoth" };
  ...
  function jsQR(data, width, height, providedOptions) {
    ...
    if (!result && (options.inversionAttempts === "attemptBoth" || options.inversionAttempts === "invertFirst" || options.inversionAttempts === "onlyInvert")) {
